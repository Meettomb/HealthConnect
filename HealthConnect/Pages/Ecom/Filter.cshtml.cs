...
namespace HealthConnect.Pages.Ecom
{
    public class FilterModel : PageModel
    {
        ...
        public IActionResult OnGet()
        {
            ...
            int? Filter = null;
            if (Request.Query.ContainsKey("Filter") && int.TryParse(Request.Query["Filter"], out int FilterName))
            {
                Filter = FilterName;
            }
            OnGetFilterPharmacy(Filter);
            return Page();
        }
        ...
        private void OnGetFilterPharmacy(int? Filter)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
            ...
            WHERE (@Filter IS NULL OR MSC.medicine_sub_category_id = @Filter OR MFC.medicine_finel_category_id = @Filter)";
                using (SqlCommand command = new SqlCommand(query, connection))
                {
